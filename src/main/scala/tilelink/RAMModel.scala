...
class TLRAMModel(log: String = "", ignoreCorruptData: Boolean = false)(implicit p: Parameters) extends LazyModule
{
      ...
      when (d_fire) {
        ...
        when (d_flight.opcode === TLMessages.ArithmeticData || d_flight.opcode === TLMessages.LogicalData) {
          ...
          when (d_last) {
            ...
            val corrupt = Bool(ignoreCorruptData) && d.corrupt
...
object TLRAMModel
{
  def apply(log: String = "", ignoreCorruptData: Boolean = false)(implicit p: Parameters): TLNode =
  {
    val model = LazyModule(new TLRAMModel(log, ignoreCorruptData))
    model.node
  }
